...
class Heaviside(UserExpression):
    def eval(self, values, x):
        level_set = 0.2 - \
            np.sqrt((x[0]-0.6)*(x[0]-0.6) + (x[1]-0.5)*(x[1]-0.5))
...
# Define boundary
noslip = DirichletBC(
    V_v, (0, 0), "near(x[0],1) || near(x[0],0) || near(x[1],0)")
upflow = DirichletBC(V_v, (1, 0), "near(x[1],1)")
pinpoint = DirichletBC(V_s, 0, "near(x[0],0) && near(x[1],0)", "pointwise")
bcu = [noslip, upflow]                                     # velocity condition
bcp = [pinpoint]                                           # pressure condition
bch = [DirichletBC(V_s, 0,                "on_boundary")]  # heaviside function
bcf = [DirichletBC(V_v, (0, 0),           "on_boundary")]  # force function
bcg = [DirichletBC(V_t, ((0, 0), (0, 0)), "on_boundary")]  # deformation gradient
...
g = project(as_matrix(((h, Constant(0)), (Constant(0), h))), V_t)
f = Function(V_v)
...
# Directory name
directory = "results2D/meshsize_" + str(n_mesh) +"/mu_s_" + str(mu_s)
...
navier_stokes_solver = ProjectSolver(u, p, f, dt=dt, nu=nu)
